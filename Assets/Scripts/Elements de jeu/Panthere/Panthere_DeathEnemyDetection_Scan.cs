...
public class Panthere_DeathEnemyDetection_Scan : MonoBehaviour
{
	...
	Panthere_Poursuite PantherePoursuite;
    ...
    void Start()
    {
		...
		PantherePoursuite = GetComponentInParent<Panthere_Poursuite> ();
        ...
        for (int i = 0; i < previousNumberOfRays; i++)
        {
            ...
            if (Physics.Raycast(viewTransform.position, currentDirection, out hit, viewDistance, obstacleMask))
            {
				...
				if(hit.transform.gameObject.tag == "Player") //si le collider touché a pour tag Player, envoie un message de collision au premier enfant du GameObject
				{
					...
					transform.GetComponentInParent<Panthere_DetectPlayer> ().SendMessage ("OnTriggerStay", hit.transform.gameObject.GetComponent<Collider> ());
					PantherePoursuite.Fuite ();

				}

				if(hit.transform.gameObject.tag == "Garde_GMB") //si le collider touché a pour tag Player, envoie un message de collision au premier enfant du GameObject
				{
