...
public class Panthere_EnemyDetection_Scan : MonoBehaviour
{
    ...
    private void UpdateSightMesh()
    {
        for (int i = 0; i < previousNumberOfRays; i++)
        {
            ...
            if (Physics.Raycast(viewTransform.position, currentDirection, out hit, viewDistance, obstacleMask))
            {
				...
				{
					transform.GetComponentInParent<Panthere_Poursuite> ().Cible = hit.transform.gameObject;
				}

				if(hit.transform.gameObject.tag == "Garde_GMB")
				{
					transform.GetComponentInParent<Panthere_Poursuite> ().Cible = hit.transform.gameObject;
				}
